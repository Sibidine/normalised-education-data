create or replace procedure populate_secondary()
as $$
declare
	country record;
	yr record;
		...
		loop
			...
			loop
				...
				loop
                                val_query := 'select "'
                                || yr.years::text
                                || '"as value from secondary_inter where country_code = '
                                || quote_literal(country.code::text)
                                ...
                                then
                                execute format('update secondary set '
                                || col.c_name::text
                                || ' = '
				...
				execute format('update secondary set upper_lower='
							  || quote_literal(cat.category::text)
							  || ' where year = '
							  || yr.years::text 
							  || ' and country_code = '
