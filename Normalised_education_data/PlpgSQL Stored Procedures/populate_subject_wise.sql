create or replace procedure populate_subjects()
as $$
declare
	country record;
	yr record;
		...
		loop
			...
			loop
				...
				loop
                                val_query := 'select "'
                                || yr.years::text
                                || '"as value from subject_inter where country_code = '
                                || quote_literal(country.code::text)
								...
								then
                                execute format('update subject_wise set '
                                || col.c_name::text
                                || ' = '
				...
				execute format('update subject_wise set subject='
							  || quote_literal(cat.category::text)
							  || ' where year = '
							  || yr.years::text 
							  || ' and country_code = '
